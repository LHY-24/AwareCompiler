"""
Intelligent LLVM Pass Sequence Recommendation Tool 

This tool provides feature matching and LLVM pass sequence recommendation based on the autophase_features.md knowledge base.
"""
...
# Set environment variables to avoid issues with flash attention and xformers in transformers
...
class KnowledgeTool(Tool):
    ...
    This class provides feature extraction, similarity calculation, and pass sequence recommendation.
    """
    ...
    def __init__(self, knowledge_base_path: str = None):
        ...
        Initialize the tool.
        Args:
            knowledge_base_path: Path to the knowledge base directory or file.
        """
        ...
        parameters = {
            ...
            "properties": {
                "query": {
                    "type": "string",
                    "description": (
                        "A string containing the autophase features of the LLVM IR program. "
                        "This should include key-value pairs for features such as TotalInsts, NumCallInst, etc. "
                        "You can provide the features in JSON format or as a plain text list. "
                        "Example (JSON): '{\"TotalInsts\": 180, \"NumCallInst\": 45, \"NumLoadInst\": 32}' "
                        "Example (plain text): 'TotalInsts: 180, NumCallInst: 45, NumLoadInst: 32'"
                    )
                }
            },
            "required": ["query"],
            "description": (
                "Call this tool to get an intelligent LLVM pass sequence recommendation based on program features. "
                "You must provide a query string containing the autophase features of your program. "
                "The tool will analyze the features and return an recommended pass sequence. "
                "To use this tool, the 'query' field should contain the program's autophase features, either as plain text or JSON format.\n"
                '{"name": "lightrag_compiler_optimization", "arguments": {"query": "<autophase features as plain text or JSON>"}}\n'
                "Example:\n"
                '{"name": "lightrag_compiler_optimization", "arguments": {"query": "Total instructions: 482\\nTotal blocks: 73\\nMemory instructions: 306\\nBranch count: 63"}}\n'
            )
        }
    ...
    def _load_autophase_knowledge(self) -> List[Dict]:
        """
        Load the autophase_features.md knowledge base file and parse its entries.
        Returns:
            List of knowledge base entries, each as a dictionary.
        """
        knowledge_entries = []
        
        try:    
            file_path = self.knowledge_base_path / "docs" / "autophase_features.md"      
            ...
            # Split the content into entries by section header
            entries = re.split(r'\n### [^\n]+\n', content)
            ...
            for entry in entries:
                ...
                # Extract performance improvement value
                ...
                # Extract optimal pass sequence (as JSON)
                ...
                # Extract autophase features table
                ...
                # Only keep entries with all required fields
    ...
    **Selection Rationale:**
    This program was selected using a composite scoring algorithm that balances feature similarity ({similarity:.1%}) and performance improvement ({performance:.1%}).
    """
...
if __name__ == "__main__":
    ...
    test_query_2 = """Act as a compiler optimization expert finding an optimal pass sequence for LLVM IR, aiming to reduce the total instruction count.
...
When you invoke the instrcount tool, you must provide the cbench-v1_adpcm.ll as the filename and the optimization flags as the optimization_flags.
"""
    
    # print("Test query:", test_query_2)
    result = tool.execute({"query": test_query_2})
    print("\nEnhanced prompt:")
    print(result)